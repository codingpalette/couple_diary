...
# 유저 생성
@router.post('')
def user_create(req: user.UserCreate, db: Session = Depends(get_db)):
    ...
    if user_login_id_info:
        raise HTTPException(status_code=401, detail={"result": "fail", "message": "이미 존재하는 아이디 입니다."})
    user_nickname_info = crud_user.get_user_by_nickname(db, req)
    if user_nickname_info:
        raise HTTPException(status_code=401, detail={"result": "fail", "message": "이미 존재하는 닉네임 입니다."})
...
@router.post('/login')
def user_login(req: user.UserLogin, db: Session = Depends(get_db)):
    user_login_id_info = crud_user.get_user_by_login_id(db, req)
    if not user_login_id_info:
        raise HTTPException(status_code=401, detail={"result": "fail", "message": "존재하지 않는 아이디 입니다."})
    else:
        ...
        if not password_check:
            ...
            if token_update:
                ...
                raise HTTPException(status_code=401, detail={"result": "fail", "message": "로그인에 실패 했습니다."})
