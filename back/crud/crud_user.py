...
def get_user_by_login_id(db: Session, req: user.UserLoginId) -> User:
    return db.query(User).filter(User.login_id == req.login_id).first()


def user_create(db: Session, req: user.UserCreate) -> User:
    db_obj = User(
...
def token_update(db: Session, user: user.UserLoginId, refresh_token: str) -> User:
