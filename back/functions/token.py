...
def create_token(type, user_info = None):
    ...
    if type == "access_token":
        ...
        payload = {"email":user_info['email'], "level":user_info['level'], 'exp': datetime.datetime.utcnow() + datetime.timedelta(days=1)}
    else:
    ...
    try:
        ...
        if not db_token:
            ...
            try:
                ...
                access_payload = { "username":db_token["username"], "level":db_token["level"], "company_id":db_token["company_id"], 'exp': datetime.datetime.utcnow() + datetime.timedelta(days=1)}
                alg = 'HS256'
