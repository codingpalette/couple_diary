...
def create_token(type, user_info = None):
    ...
    if type == "access_token":
        ...
        payload = {"id": user_info['id'], "email": user_info['email'], "name": user_info['name'], "level": user_info['level'], 'exp': datetime.datetime.utcnow() + datetime.timedelta(days=1)}
    else:
    ...
    try:
        ...
        if not db_token:
            ...
            try:
                ...
                access_payload = {"id": db_token['id'], "email": db_token["email"], "name": db_token["name"], "level": db_token["level"], 'exp': datetime.datetime.utcnow() + datetime.timedelta(days=1)}
                alg = 'HS256'
